...
def compress_image(file_path, output_dir, processed_images):
    """压缩单个图片文件"""
    try:
        # 计算输出文件的相对路径
        ...
        logger.info(f"✅ 成功压缩: {file_path} → {output_path}")
        ...
        logger.error(f"⚠️ 处理错误: {file_path}\n原因: {str(e)}")
        return False
...
def main():
    ...
    # 设置命令行参数
    parser = argparse.ArgumentParser(description='多线程图片压缩工具 - 仅处理DSC开头的文件')
    ...
    parser.add_argument('-j', '--jobs', type=int, metavar='NUM', 
                        ...
                        help=f'并行任务数 (默认: CPU核心数={os.cpu_count()})')
    ...
    # 获取所有符合条件的图片文件
    image_files = []
    prefix_pattern = re.compile(f"^{re.escape(args.prefix)}", re.IGNORECASE)
    
    for ext in image_extensions:
        # 递归查找所有匹配扩展名的文件
        for file_path in input_root.rglob(f"*{ext}"):
            if file_path.is_file() and prefix_pattern.match(file_path.name):
                image_files.append(file_path)
        
        # 处理大写扩展名
        for file_path in input_root.rglob(f"*{ext.upper()}"):
            if file_path.is_file() and prefix_pattern.match(file_path.name):
                image_files.append(file_path)
    
    # 去重（某些文件可能同时匹配小写和大写扩展名）
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=jobs) as executor:
        ...
        for file in image_files:
            future = executor.submit(compress_image, file, output_dir, processed_images)
